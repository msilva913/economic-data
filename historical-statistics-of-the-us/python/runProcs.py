...
def tex(x=None):
    ...
    for files in os.listdir('.'):
        if files.endswith('.aux') or files.endswith('.log') or files.endswith('.out') or files.endswith('.gz') or files.endswith('.snm') or files.endswith('.nav') or files.endswith('.toc') or files.endswith('.blg') or files.endswith('.bbl'):
            os.remove(files)
...
def pdfLatex(fileName):
    ...
    pdflatex_plus_texfile = 'pdflatex '+fileName
    if fileName.endswith('.tex')==False:
        pdflatex_plus_texfile = pdflatex_plus_texfile+'.tex'

    bibtex_plus_texfile = pdflatex_plus_texfile.replace('.tex','.aux').replace('pdflatex','bibtex')
    subprocess.call(pdflatex_plus_texfile,shell=True,stdout=FNULL)
    subprocess.call(pdflatex_plus_texfile,shell=True,stdout=FNULL)
    subprocess.call(bibtex_plus_texfile,shell=True,stdout=FNULL)
    subprocess.call(bibtex_plus_texfile,shell=True,stdout=FNULL)
    subprocess.call(pdflatex_plus_texfile,shell=True,stdout=FNULL)
    subprocess.call(pdflatex_plus_texfile,shell=True,stdout=FNULL)
...
def handout(fileName):
    ...
    if fileName.endswith('.tex'):
        with open(fileName) as oldLines, open('Handout'+fileName[6:], "w") as newLines:
